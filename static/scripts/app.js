...
/**
 * 同步网格背景与相机位置，使网格点与世界坐标对齐
 */
function syncGridToCamera() {
  const grid = document.getElementById('grid');
  if (!grid) return;
  const bgX = ((-camera.x * zoom) % GRID_SIZE + canvas.width % GRID_SIZE) % GRID_SIZE;
  const bgY = ((-camera.y * zoom) % GRID_SIZE + canvas.height % GRID_SIZE) % GRID_SIZE;
...
async function init() {
  ...
  document
    ...
    .addEventListener('click', () => setTool('select', 'btn-select', '选择工具已激活'));
  document
    .getElementById('btn-input-toggle')
    .addEventListener('click', () => setTool('input-toggle', 'btn-input-toggle', '输入切换工具已激活'));
  document
    .getElementById('btn-delete')
    .addEventListener('click', () => setTool('delete', 'btn-delete', '删除工具已激活'));
  ...
  function clearToolbarActive() {
    document.querySelectorAll('.toolbar button').forEach((btn) => {
      if (btn.id === 'btn-wire-anim') return;
  ...
  function handleKeyDown(e) {
    // 如果焦点在文本输入框中（Agent 输入框、注释弹窗等），放行编辑快捷键
...
function addElement(type) {
  ...
  if (element) {
    const desiredX = snapToGrid(worldX - element.width / 2);
    const desiredY = snapToGrid(worldY - element.height / 2);
...
function buildSignalAnimation(sourceElementId, beforeSnapshot, targetSnapshot) {
  ...
  while (true) {
    ...
    for (const info of outs) {
      ...
      const nextTime =
        currentTime + SIGNAL_ANIMATION_SETTINGS.nodeEmitDelay + travel + SIGNAL_ANIMATION_SETTINGS.nodeLightDelay;
...
function updateSignalAnimation(now) {
  ...
  while (
    signalAnimation.elementIndex < signalAnimation.elementEvents.length &&
  ...
  while (
    signalAnimation.wireIndex < signalAnimation.wireEvents.length &&
...
function handleMouseDown(e) {
  ...
  if (isPlacingFunction && currentFunctionToPlace && e.button === 0) {
    ...
    if (target && target.id === 'canvas') {
      currentFunctionToPlace.x = snapToGrid(currentFunctionToPlace.x);
      currentFunctionToPlace.y = snapToGrid(currentFunctionToPlace.y);
      moveElementWithAutoSeparation(currentFunctionToPlace, currentFunctionToPlace.x, currentFunctionToPlace.y);
  ...
  if (isPlacingElement && e.button === 0) {
    ...
    if (target && target.id === 'canvas') {
      ...
      elementToPlace.x = snapToGrid(elementToPlace.x);
      elementToPlace.y = snapToGrid(elementToPlace.y);
      moveElementWithAutoSeparation(elementToPlace, elementToPlace.x, elementToPlace.y);
  ...
  for (const element of elements) {
    if (
      ...
      if (currentTool === 'input-toggle' && element.type === 'INPUT') {
        ...
        // 删除工具：删除元件
        // 删除连接到该元件的所有导线
        wires = wires.filter((wire) => wire.start.elementId !== element.id && wire.end.elementId !== element.id);
        // 删除元件
  ...
  if (e.type === 'auxclick' && e.button === 1 && isMiddleClickCopy && currentElementToPlace) {
    ...
    currentElementToPlace.x = snapToGrid(worldX - currentElementToPlace.width / 2);
    currentElementToPlace.y = snapToGrid(worldY - currentElementToPlace.height / 2);
...
function handleMouseMove(e) {
  ...
  if (isGroupDragging && selectedElements.length > 0) {
    // Snap the group's new root position, then apply delta to each element
    const firstItem = groupDragOffsets[0];
    const newRootX = snapToGrid(worldX - firstItem.offsetX);
    const newRootY = snapToGrid(worldY - firstItem.offsetY);
    const deltaX = newRootX - firstItem.element.x;
    const deltaY = newRootY - firstItem.element.y;
  ...
  if (isDragging && selectedElement) {
    const snappedX = snapToGrid(worldX - dragOffset.x);
    const snappedY = snapToGrid(worldY - dragOffset.y);
    // Only move if position actually changed (avoid jitter when snapping)
    if (snappedX !== selectedElement.x || snappedY !== selectedElement.y) {
      moveElementWithAutoSeparation(
        selectedElement,
        snappedX,
        snappedY,
...
function deleteSelected() {
  ...
  // 删除连接到选中元件的所有导线
  wires = wires.filter(
    (wire) => !elementIds.includes(wire.start.elementId) && !elementIds.includes(wire.end.elementId),
  );
...
function copySelected() {
  ...
  clipboardWires = wires
    .filter((wire) => selectedIds.includes(wire.start.elementId) && selectedIds.includes(wire.end.elementId))
    .map((wire) => ({
      ...wire,
      id: generateId(),
      start: { ...wire.start },
  ...
  document.getElementById('status-bar').textContent =
    `已复制 ${clipboardElements.length} 个元件和 ${clipboardWires.length} 条导线，按 Ctrl+V 粘贴`;
}
...
function executePaste() {
    ...
    if (newStartElementId && newEndElementId) {
      ...
      if (startElement && endElement) {
        ...
        if (startElementTemplate && endElementTemplate) {
          ...
          const endPort = wireTemplate.end.isInput ? endElement.inputs[endPortIndex] : endElement.outputs[endPortIndex];
  ...
  elements = calculateCircuit(elements, wires);
  document.getElementById('status-bar').textContent =
    `已粘贴 ${newElements.length} 个元件和 ${clipboardWires.length} 条导线`;
  render(ctx, elements, wires, selectedElement, selectedWire, null, selectedElements, [], null, false, zoom, camera);
}
  ...
  if (e.button === 0) {
    ...
    if (isSelecting) {
      ...
      if (rect && rect.width >= 5 && rect.height >= 5) {
        ...
        if (selectedElements.length > 0) {
          document.getElementById('status-bar').textContent =
            `已选中 ${selectedElements.length} 个元件，按 Ctrl+D 删除，Ctrl+C 复制`;
        } else {
          document.getElementById('status-bar').textContent = '就绪';
        }
      }
    ...
    if (isDragging) {
      ...
      // 检查是否是点击（而非拖动）
      const moveDistance = dragStartPos
        ? Math.sqrt(Math.pow(mouseX - dragStartPos.x, 2) + Math.pow(mouseY - dragStartPos.y, 2))
        : 0;
        ...
        for (const input of element.inputs) {
          ...
          if (distance(worldX, worldY, portX, portY) < 10 / zoom) {
            ...
            if (!wireStart.isInput) {
              ...
              const startPortIndex = startElement
                ? startElement.outputs.findIndex((p) => p.id === wireStart.portId)
                : -1;
              const wire = {
                id: generateId(),
...
function updateWirePositions(wire) {
  ...
  if (wire.start.portId) {
    const startPort = [...(startElement.outputs || []), ...(startElement.inputs || [])].find(
      (p) => p.id === wire.start.portId,
    );
    if (startPort) {
  ...
  if (wire.end.portId) {
    const endPort = [...(endElement.outputs || []), ...(endElement.inputs || [])].find((p) => p.id === wire.end.portId);
    if (endPort) {
...
async function saveSelectedAsFunction(name) {
  ...
  const selectedWires = wires.filter(
    (wire) => selectedIds.includes(wire.start.elementId) && selectedIds.includes(wire.end.elementId),
  );

  // 获取输入和输出元件ID列表
  const inputElementIds = selectedElements.filter((el) => el.type === 'INPUT').map((el) => el.id);
  const outputElementIds = selectedElements.filter((el) => el.type === 'OUTPUT').map((el) => el.id);
...
function updateModulePanel() {
  ...
  savedModules.forEach((func, index) => {
    ...
    deleteBtn.style.cssText =
      'background: rgba(255,0,0,0.2); border: 1px solid rgba(255,0,0,0.4); color: #ff6666; padding: 2px 8px; border-radius: 4px; cursor: pointer; font-size: 14px; line-height: 1;';
...
function startRenderLoop() {
  function loop() {
    ...
    render(
      ...
      selectionRect,
      selectedElements,
      isPasting ? clipboardElements : [],
      isPasting ? pasteOffset : null,
      isPasting,
      zoom,
      camera,
      signalAnimation,
